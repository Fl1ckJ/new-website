...
/* ---------- supply-chain node ---------- */
function ChainNode({ seg, i, onOpen }) {
  return (
    <button className="node" style={{ "--enter-y": "18px", "--enter-o": 0, animation: `nodeIn .6s var(--ease-out) ${0.15 + i * 0.11}s forwards` }}
            onClick={() => onOpen(seg.code)}>
      {seg.flag && <span className="node-flag">{seg.flag}</span>}
      <div className="node-top">
        <span className="node-code">{seg.code}</span>
        <span className="node-stage">{String(i + 1).padStart(2, "0")} · {seg.stage}</span>
      </div>
      ...
      <div className="chain">
        {segments.map((seg, i) => (
          <React.Fragment key={seg.code}>
            <ChainNode seg={seg} i={i} onOpen={onOpen} />
            {i < segments.length - 1 && (
              <div className="connector">
                <span className="pulse" style={{ "--pd": `${i * 0.55}s` }} />
                <span className="pulse" style={{ "--pd": `${i * 0.55 + 1.6}s` }} />
              </div>
            )}
          </React.Fragment>
        ))}
...
function PersonaCard({ p, i, onOpen }) {
  return (
    <button className="pcard" style={{ animation: `nodeIn .6s var(--ease-out) ${0.12 + i * 0.08}s forwards`, "--enter-o": 0 }}
...
function Detail({ item, kind, list, onBack, onSwitch }) {
  ...
  useEffect(() => { if (scrollRef.current) scrollRef.current.scrollTop = 0; }, [item && item.code]);
  if (!item) return null;
